...
class AppURLopener(urllib.request.FancyURLopener):
    version = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:71.0) \
...
def parser(my_url):
    ...
    if "wikipedia" or "appledaily" in my_url:
        for i in page_soup.select('p'):
            plaintext = wiki_quotes_eraser(
                i.get_text().strip(), amount_of_quotes) + plaintext
    elif "fyan" in my_url:
...
dataFile = open("raw_training_data.txt", "w", encoding="utf8")
